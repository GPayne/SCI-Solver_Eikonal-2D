...
In a terminal:
```c++
mkdir SCI-SOLVER_Eikonal-2D/build
cd SCI-SOLVER_Eikonal-2D/build
cmake ../src
make
```
...
```c++
mkdir C:\Path\To\SCI-Solver_Eikonal-2D\build
cd C:\Path\To\SCI-Solver_Eikonal-2D\build
...
A basic usage of the library links to the <code>libEikonal2D_CORE</code> library during build and 
